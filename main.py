...
app.config['MAIL_USERNAME'] = 'petloverwebz@gmail.com'
app.config['MAIL_PASSWORD'] = 'petlove123'
...
def saveuserreg():
    ...
    db.saveuser(name, hname, state, district, place, pincode,
                dob, phone, email, password, category, ques, ans)
...
def saveclinicreg():
    ...
    ques = request.form['txtquestion']
    ans = request.form['txtans']
    db.saveclinic(name, reg, place, district, state, phone, pincode, email,
                  govform, txtname, address, txtphn, password, ques, ans, filename)
    return render_template("message.html", message="Registration Successfull", route="/login1")
...
def savefarmreg():
    ...
    session["farmid"] = db.savefarm(
        name, state, district, place, year, regno, phone, email, filename, session["useremail"])
...
def saveuserpetdetails():
    ...
    db.saveuserpetdetails(pname, name, breed, other, gender, age,
                          weekmonth, description, filename, session["useremail"])
...
def saveeditpet():
    ...
    description = request.form.get('txtdesc')
    db.editpets(petid, pname, name, breed, other, gender, age,
                weekmonths, description, session["useremail"], filename)
...
def saveeditfarm():
    ...
    phone = request.form.get('txtphn')
    db.editfarm(fname, state, district, place, yearr, regnum,
                phone, session["useremail"], filename)
...
def saveuseredit():
    ...
    phone = request.form.get('txtphn')
    db.edituser(name, hname, state, district, place,
                pincode, dob, phone, session["useremail"])
    return render_template("message.html", message="Updated Successfully", route="/showuserhome")
...
def savedoctoredit():
    ...
    db.editdoc(dname, hname, state, district, place, dob, phone,
               regnum, yearr, specification, session["useremail"], filename)
...
def saveliedit():
    ...
    db.editli(lname, state, district, place, dob, phone,
              pen, session["useremail"], filename)
...
def savefarmappointment():
    ...
    db.savefarmappointment(
        clinic, res[0], res[1], reason, date, time, session["useremail"], session["farmid"])
...
def acceptclinic():
    ...
    msg = Message("PET SAKE | Account Verification",
                  sender="petsake123@gmail.com", recipients=[email])
...
def saveattenderreg():
    ...
    email = request.form['txtemail']
    password = request.form['txtpswrd']
    db.saveattender(name, gender, state, district, place, date,
                    phone, email, password, session["useremail"])
...
def savedoctorreg():
    ...
    ques = request.form['txtquestion']
    ans = request.form['txtans']
    msg = Message("Heloo your username is "+email+" and password is " +
                  password, sender="petsake123@gmail.com", recipients=[email])
    # Mail.send(message=msg)
    db.savedoctor(name, gender, hname, state, district, place, date, phone, email,
                  regnum, yor, spec, password, ques, ans, filename, session["useremail"])
    msg = Message("PET SAKE | Login Credentials",
                  sender="petsake123@gmail.com", recipients=[email])
    msg.body = "Hello welcome to PETSAKE,your username is " + \
...
def savelireg():
    ...
    ques = request.form['txtquestion']
    ans = request.form['txtans']
    db.saveli(name, gender, state, district, place, date, phone, pen,
              email, password, ques, ans, filename, session["useremail"])
    msg = Message("PET SAKE | Login Credentials",
                  sender="petsake123@gmail.com", recipients=[email])
    msg.body = "Hello welcome to PETSAKE,your username is " + \
        email+" and password is "+password
...
def liinfoupload():
    ...
    db.liinfo(name, filename, datpgm, description,
              postdat, postperson, session['useremail'])
...
def savechangepasswordu():
    ...
    db.savechangepasswordc(session["useremail"],
...
def getnewmessage():
    ...
    if(last_raw != False):
        ...
        if(last_msg != prev_msg):
            prev_msg = last_msg
            retmsg = {"id": last_raw[0], "message": last_raw[3],
                      "from_id": last_raw[1], "to_id": last_raw[2]}
